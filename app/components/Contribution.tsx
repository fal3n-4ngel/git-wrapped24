...
const ContributionDashboard: React.FC<ContributionData> = ({
  ...
  const processedContributions = useMemo(() => {
    return Object.entries(contributions).map(([date, contributions]) => ({
      date,
      contributions
  ...
  const stats = useMemo(() => {
    ...
    return {
      totalContributions,
      averageDaily: Number((totalContributions / contributionValues.length).toFixed(2)),
    ...
    <div className="bg-[#3d3d3d] text-slate-200 font-mono p-8 space-y-8">
      ...
      <div className="grid grid-cols-3 gap-6">
        {[{
          ...
          <div key={index} className="bg-[#2d2d2d] border border-slate-400 p-4 rounded-lg">
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-[#2d2d2d] border border-slate-400 p-4 rounded-lg">
          <h3 className="text-lg mb-4 text-slate-200">Cumulative Contributions</h3>
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={processedContributions.map((item, index) => ({
              ...item,
              cumulative: processedContributions.slice(0, index + 1)
    ...
    const asciiGraphLines = useMemo(() => {
      ...
      return filteredContributions.map(({ date, contributions }) => {
        const bar = Array.from({ length: contributions }).map((_, idx) => (
          <span key={idx} className="w-1 h-1">▓</span>
        ));
  
        return (
          <div
            key={date}
            className="text-sm font-mono bg-[#2d2d2d] text-slate-200"
          >
